...
get_resources () {
	RESPONSE=`curl -s "https://panel.cloudatcost.com/api/v1/cloudpro/resources.php?key=$KEY&login=$MAIL"`
	check_response
...
list_servers () {
	RESPONSE=`curl -s -X GET "https://panel.cloudatcost.com/api/v1/listservers.php?key=$KEY&login=$MAIL"`
	check_response
...
list_tasks () {
	RESPONSE=`curl -s -X GET "https://panel.cloudatcost.com/api/v1/listtasks.php?key=$KEY&login=$MAIL"`
	check_response
	echo $RESPONSE | jq .
}
show_resources () {
...
if [ $OPETYPE = "b" ] || [ $OPETYPE = "B" ]; then
	if [ -z "$OS" ]; then
		...
		read DC
		echo "OS List"
		RESPONSE=`curl -s "https://panel.cloudatcost.com/api/v1/listtemplates.php?key=$KEY&login=$MAIL"`
		check_response
	...
	RESPONSE=`curl -s -X POST https://panel.cloudatcost.com/api/v1/cloudpro/build.php --data "key=$KEY&login=$MAIL&datacenter=$DC&cpu=$CPU&ram=$RAM&storage=$STORAGE&os=$OS"`
	check_response
	echo $RESPONSE | jq .
	...
	if [ $CONFIRM = "y" ] || [ $CONFIRM = "Y" ]; then
		RESPONSE=`curl -s -X POST https://panel.cloudatcost.com/api/v1/cloudpro/delete.php --data "key=$KEY&login=$MAIL&sid=$SID"`
		check_response
		echo $RESPONSE | jq
	fi
	...
	read ACTION
	RESPONSE=`curl -s -X POST https://panel.cloudatcost.com/api/v1/powerop.php --data "key=$KEY&login=$MAIL&sid=$SID&action=$ACTION"`
	check_response
	echo $RESPONSE | jq .
	...
	RESPONSE=`curl -s -X POST https://panel.cloudatcost.com/api/v1/runmode.php --data "key=$KEY&login=$MAIL&sid=$SID&mode=$MODE"`
	echo $RESPONSE | jq .
